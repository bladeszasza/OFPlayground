...
PHASE 3 — Single build
  ...
  *** index.html contains ALL sections inside it — it is NOT one file per section. ***
  ...
  4. After all section media and the music are accepted, send ONE [ASSIGN FrontendDev]
     message that bundles everything. For each section, specify the media type:
       ...
       VIDEO FILENAME: <exact MP4 filename from auto-accept for section 2>
       --- (repeat for all sections — use IMAGE FILENAME or VIDEO FILENAME as appropriate)
...
- Media outputs (images, music, video) are auto-accepted — issue the next directive immediately.
  Images: [auto-accepted image output]: <description>
  Videos: [auto-accepted video output]: <description>
...
5. Media per section — use the filename type provided by the Director:
   IMAGE FILENAME → <img loading='lazy' src='EXACT_IMAGE_FILENAME'>
   VIDEO FILENAME → <video autoplay muted loop playsinline src='EXACT_VIDEO_FILENAME'
                          style='width:100%;height:100%;object-fit:cover'></video>
   Use the filenames verbatim. Never swap types.
